...
def convert_to_mesh(cloth,np_mesh):
    mesh = bpy.data.meshes.new(name="MyMesh")
    bm = bmesh.new()
    bm.from_mesh(cloth.data)
    try:
        for i, v in enumerate(bm.verts):v.co = Vector(mesh[i])
    except:
        pass
    bm.to_mesh(mesh)
    return mesh
...
class ClothManipulationStep(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        print('picking by coordinates:', co_picks[ms])
        print('slide by:', move)
...
class ClothManipulationSequence(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        file_index = 0
        dir_str = base_dir+context.scene.data_dir+'/'#+mode_str+'/'
...
class ClothManipulationPlayback(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        if context.scene.inferred_prop:
            ...
            try:
                #with np.load(plan_file_name) as plans:
                ...
                #print('plans moves shape:',plan_moves.shape)
                #print('plans angles shape:',plan_angles.shape)
                #print('plans mirror shape:',plan_mirror.shape)
                #n_data = plan_moves.shape[0]
        ...
        for data_index in range_data:
            ...
            for seq_len in range(1,max_sequence_length+1):
                ...
                for from_step in range(max_sequence_length-seq_len+1):
                    ...
                    print('min/max',mesh[:,0].min(),mesh[:,0].max())
                    
                    # rotate mesh
                    if context.scene.inferred_prop:
                        #rot = plan_angles[data_index,counter]
                        try:
                    ...
                    if context.scene.inferred_prop:
                        #moves = plan_moves[data_index,counter,from_step:from_step+seq_len]
            ...
            #if not context.scene.playback_single:
...
class PingPong(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        data_set = context.scene.data_subset
        output_path = base_dir+'/'+run_name+'/eval/pingpong/'
        os.makedirs(output_path,exist_ok=True)
        #output_path = output_path+'/'+data_set
        #os.makedirs(output_path,exist_ok=True)
        ...
        for data_index in range_data:
            ...
            for seq_len in range(1,max_sequence_length+1):
                ...
                for from_step in range(max_sequence_length-seq_len+1):
                    ...
                    for pp_step in xrange(seq_len):
                    
                        # get planning data
                        identifier = data_set+'_'+str(data_index).zfill(5)+'_'+str(seq_len)+'_'+str(from_step)+'_'+str(step)
                        plan_file_name = output_path+identifier+'_'+str(pp_step)+'p.pickle'
                        while not os.path.exists(outcome_file_path):
                            print('waiting for file:',plan_file_name)
                            time.sleep(1)
                        print('load file:',plan_file_name)
                        try:
                            pp_state = pickle.load(open(plan_file_name,'rb'),encoding='latin1')
                        ...
                        i = 0
                        picks = np.array([[[moves[s,1],moves[s,0]],[moves[s,3],moves[s,2]]] for s in range(1)]) # simplify this mess
                        slide = np.array([[moves[s,5],moves[s,4]] for s in range(1)]) # simplify this mess
                        ...
                        # save ping pong state
                        plan_file_name = output_path+identifier+'_'+str(pp_step)+'b.pickle'
                        with open(plan_file_name,'wb') as outfile:
                            pickle.dump(pp_state,outfile,protocol=2)
...
def register():
    ...
    bpy.types.Scene.edge_mode = bpy.props.BoolProperty(name = "Edge mode",
        description = "Whether to pick anywhere or just at the edges (of the cloth's silhouette)",default = True)
    ...
    # save dir entry
    #bpy.types.Scene.data_dir = bpy.props.StringProperty(name = "Save dir",
    #    description = "Directory to save generated examples to",default = '<dir_name>')
